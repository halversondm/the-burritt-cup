...
const navModeClasses: Record<NavMode, string> = {
  ...
  fixed:
    "-translate-y-full bg-cream-50/95 text-charcoal-900 shadow-md backdrop-blur-sm",
  "fixed-visible":
    "translate-y-0 bg-cream-50/95 text-charcoal-900 shadow-md backdrop-blur-sm",
};

const navLinkClass =
  "px-3 py-2 font-sans text-xs font-bold uppercase tracking-widest hover:text-gold-500";
...
export default function App() {
  ...
  return (
    <div>
      <nav
        className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${navModeClasses[state.navMode]}`}
      >
      ...
      <footer className="py-12">
        <div className="mx-auto flex max-w-5xl flex-col items-center gap-4 px-6">
          ...
          <p className="font-sans text-sm text-sage-500">
            Copyright &copy; The Burritt Cup {new Date().getFullYear()}
          </p>
        </div>
      </footer>
    </div>
